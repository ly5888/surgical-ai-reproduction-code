...
__all__ = ['ResNet', 'resnet50','res50_encoder', 'ASPP', 'Bottleneck']
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        self.z = x4
        x_aspp = self.aspp(x4)
        x_out = nn.Upsample(size, mode='bilinear', align_corners=True)(x_aspp)
        return x_out
...
def resnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        print("[Load Networks] load network resnet from ImageNet")
    return model

def res50_encoder(pretrained=True, **kwargs):
